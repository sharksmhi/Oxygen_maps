    ...
    data["DOXY_ml"] = np.where(
        ...
        valid_h2s, 0.01,
            ...
            np.where((~valid_h2s) & (below_det_btl), 0.01,
                ...
                np.where((valid_btl),
                    data['Dissolved oxygen O2 bottle (ml/l)'],
                    # O2 CTD exists and O2 CTD is not S gives O2 CTD
                    np.where((valid_ctd),
                        data['Dissolved oxygen O2 CTD (ml/l)'],
...
def DIVA_neg_oxygen(data: pd.DataFrame):
    ...
    # Apply nested np.where for all conditions
    data["NEG_DOXY_ml"] = np.where(
        # h2s valid-> h2s default (0.01)
        valid_h2s, data["Hydrogen sulphide H2S (umol/l)"] * -0.04488,
            # h2s not valid and o2< gives h2s default (0.01)
            np.where((~valid_h2s) & (below_det_btl), 0.01,
                #  o2 BTL is valid gives o2 BTL
                np.where((valid_btl),
...
df_filtered[column_list].to_csv(file_path+"sharkweb_btlctd_02_241107.txt", index=False, header=False, sep='\t')
df_filtered[column_list_test].to_csv(file_path+"sharkweb_btlctd_02_241107_test.txt", index=False, header=True, sep='\t')
