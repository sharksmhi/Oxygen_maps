...
for i in 1:length(monthlist)
    ...
    function plotres(timeindex,selected,fit,erri)
        ...
        title("lx=$(lx)[m],ly=$(ly)[m], lz=$(lenz[1,1,depth_index[1]])[m],e=$(epsilon2)")
        ...
        @sprintf("_%f",epsilon2) * @sprintf("_%05d",lx) * @sprintf("_%d",lenz[1,1,depth_index[1]]) *
    ...
    filename = joinpath(outputdir, "$(replace(varname,' '=>'_'))_$(seasons[i])_$(yearlist[i])_$(bath_file_name)")
    ...
    @time dbinfo = diva3d((lonr,latr,depthr,TS),
              ...
              transform = Anam.loglin(epsilon2),   
              mask = new_mask,
              solver = solver,
              niter_e = 1,
